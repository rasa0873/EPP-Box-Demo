...
class MyWorkoutListActivity : BaseActivity(), MyWorkoutListAdapter.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupActionBar()
        showProgressBarStandAlone(true)
    ...
    fun populateWorkoutListToUI(workoutList: ArrayList<Workout>) {
        showProgressBarStandAlone(false)
        hideProgressDialog()
        ...
        if (filteredList.size > 0){
            ...
            val adapter = MyWorkoutListAdapter(this, filteredList, currentTimestamp)
    ...
    fun showProgressBarStandAlone(show: Boolean, showError: Boolean = false) {
        if (show){
